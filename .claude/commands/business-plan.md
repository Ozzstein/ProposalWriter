...
5. **Phase 0.5 — Discovery interview (INTERACTIVE — orchestrator-driven, NOT spawned)**:
   - If `--skip-interview`, skip this phase and note the skip in `decision_log.jsonl`.
   - If `intermediate/business_plan_interview.json` already exists and `--restart-interview` was not passed, load answered questions and resume from the first unanswered.
   - Otherwise: read `agents/workers/business_plan/bp_interviewer.md` as a protocol/question-bank. Execute the 5 batches directly in the main conversation (do NOT spawn a Task-tool agent — it cannot interact with the user).
   - Use `AskUserQuestion` (loaded via ToolSearch `select:AskUserQuestion`) for multi-choice items; use plain free-text prompts for open-ended items. Present each batch as a block of 5–8 questions — do NOT ask one question per turn.
   - Accept `defaults` (accept all defaults for the batch), `skip` (mark the question `skipped`), `not yet known`, `CFO-scope`, or free-text.
...
10. **Suggest next step**: typically `/figures F-07,F-08` to produce the BP-specific diagrams; or `/finance --round N+1` once the CFO closes RC Calculator roadblockers (then re-run `/business-plan --round N+1` to close the CFO stubs); or `/review` for a holistic cross-document consistency pass; or `/business-plan --restart-interview` to re-shape the plan from new user input.

## Quick reference
- Orchestrator: `agents/orchestrators/business_plan_orchestrator.md`
- Interview protocol / question bank: `agents/workers/business_plan/bp_interviewer.md` (executed by the orchestrator directly in the main conversation, NOT spawned via Task tool)
...
- Default output paths: `runs/{project}/intermediate/business_plan_{interview.json,facts.json,gaps.md,inventory.json}`, `runs/{project}/drafts/BP_{01..04}_*.md`, `runs/{project}/drafts/business_plan_assembled.md`, `runs/{project}/final/{project}_Business_Plan.docx`, `runs/{project}/reviews/business_plan_review_{round}.json`

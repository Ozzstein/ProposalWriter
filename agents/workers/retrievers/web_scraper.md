...
Use Firecrawl CLI. Write all output to `.firecrawl/` to avoid context window bloat.
...
- **high**: Published open-access (PeerJ, Europe PMC, HAL) with peer review noted in content; or CORDIS final project result with EC-validated deliverable
- **medium**: Preprint with a DOI or journal reference in the text; Zenodo record with cited methods paper; OpenAIRE record linked to a journal article
- **low**: Preprint-only (bioRxiv, medRxiv, EarthArXiv) with no journal acceptance signal; CORDIS mid-project deliverable or working paper
